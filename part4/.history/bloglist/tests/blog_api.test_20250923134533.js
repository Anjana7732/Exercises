...
// 4.12: POST missing title or url returns 40
...
describe('deleting a blog', () => {
  ...
  test('fails with status 404 if blog does not exist', async () => {
    const validNonexistingId = await new Blog({ title:'temp', url:'http://x.com' }).save().then(b => {
      return b.id
    })

    await Blog.findByIdAndDelete(validNonexistingId) // delete it manually
...
describe('updating a blog', () => {
  ...
  test('fails with status 404 if blog does not exist', async () => {
    const validNonexistingId = await new Blog({ title:'temp', url:'http://x.com' }).save().then(b => b.id)
    await Blog.findByIdAndDelete(validNonexistingId)
    await api.put(`/api/blogs/${validNonexistingId}`).send({ likes: 10 }).expect(404)
  })
})
