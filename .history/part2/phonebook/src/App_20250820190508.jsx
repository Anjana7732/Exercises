...
const App =() =>{
  ...
  const handleClick =(e) => {
    ...
    showNotification(`${trimmedName} is already added to phonebook`, 'error');      return;
    }
    if(persons.some(person => person.name === trimmedName && person.number!== trimmedNumber)){
  ...
  const handleDel =(id, name) =>{
    ...
    if(confirm) {
      ...
      setPersons(updated)
      showNotification(`${name} was deleted.`, 'error')
    }
  }
  ...
  return (
    <div>
      <h2>Phonebook</h2>
      <Notification message={notification.message} type={notification.ty}/>
      <form>
        <div>
          Filter shown with: 
