...
const App = () => {
  ...
  console.log("showNotification called with:", { message, typ
  ...
  if (existing) {
    if (window.confirm(`${name} exists. Replace number?`)) {
      axios
        ...
        .catch((error) =>
          showNotification(error.response?.data?.error || "Error updating", "error")
        );
    }
    return;
  }
  ...
  axios
    ...
    .catch((error) =>
      showNotification(error.response?.data?.error || "Error adding", "error")
    );
};
